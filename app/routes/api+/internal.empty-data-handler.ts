...
export async function action({ request }: { request: Request }) {
  ...
  try {
    for (const entity of entities) {
      ...
      console.log(
        `Entity ${entity}: Current active version: ${current?.activeVersion}, New version will be: ${newVersion}`,
      );
      ...
      await prisma.$transaction(async (tx) => {
        ...
        try {
          ...
          await tx.$executeRawUnsafe(
            `DROP TABLE IF EXISTS "${entity}_current"`,
          );
        }
    ...
    return Response.json({
      success: true,
      message:
        "Successfully created empty tables and updated views for all entities",
